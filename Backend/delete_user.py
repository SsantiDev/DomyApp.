...
def delete_user_and_data(email):
    try:
        ...
        # Confirmation count
        related_data = []
        if hasattr(user, 'profile'): related_data.append("Profile")
        if hasattr(user, 'worker_info'): related_data.append("Worker Profile")
        if hasattr(user, 'verification'): related_data.append("Verification docs")
        
        # Service requests where they are client
        try:
            sr_count = user.service_requests_as_client.count()
            if sr_count > 0: related_data.append(f"{sr_count} Service Requests (as client)")
        ...
        try:
            ...
            if msg_count > 0: related_data.append(f"{msg_count} Sent Messages")
        except AttributeError:
            pass
